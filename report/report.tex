...
Large language models (LLMs) are increasingly used for code generation and the automation of software development, yet most evaluations focus on high-level programming languages such as Python. This study extends the work \citetitle{llmgen2025} by~\citeauthor{llmgen2025}~\cite{llmgen2025} by examining whether state-of-the-art LLMs are capable of generating efficient Java implementations of graph algorithms when provided only with the C language source files as context. Our benchmark comprises three LLMs (Anthropic Claude Opus 4.6, Google Gemini 3.1 Pro, and OpenAI GPT-5.3-Codex) and their code generation is evaluated on two different approaches similar to the previous work. The first optimization approach queries the LLMs to generate the most efficient routine for the problem of finding the number of triangles in a graph, while being given a series of human-written C implementations that solve this problem. The second algorithm-synthesis approach requires the LLMs to generate Java implementations for Triangle Counting, Graph Diameter, and Clique Number, where the models are solely provided with the benchmark suite in C and need to make assumptions about an equivalent benchmark suite in Java, without actually seeing it. The implementations are evaluated based on compilability, correctness, runtime efficiency, and memory footprint using synthetic RMAT benchmark graphs. Our results reveal significant differences between the evaluated models in the cross-language setting. While the optimization approach showed broad success across all models, the algorithm-synthesis approach was highly dominated by Google's Gemini 3.1 Pro model with 100\% RTU implementations. Both other models struggled with API consistency and therefore performed poorly on the second approach. These findings highlight the difficulty of structural abstraction in LLM code generation and suggest that current efficiency metrics are highly sensitive to small-sample evaluation suites.
...
Large language models (LLMs) have seen broad adoption across nearly every sector, assisting with tasks ranging from natural language understanding to complex reasoning and software development~\cite{chang2024survey}. One of their most impactful applications is code generation, where they help programmers by automating development tasks and reducing human error~\cite{llmgen2025}. While most existing benchmarks such as HumanEval~\cite{chen2021evaluatinglargelanguagemodels} and MBPP~\cite{austin2021programsynthesislargelanguage} focus primarily on correctness and functionality, the performance of written code in terms of memory footprint and runtime efficiency is largely overlooked. However, in scientific computing and graph analysis small algorithmic optimizations can result in a reduction of runtime and energy consumption by orders of magnitude~\cite{llmgen2025}. Barati Nia, Dindoost, and Bader~\cite{llmgen2025} addressed this by benchmarking eight LLMs on their ability to generate efficient C implementations of graph algorithms, supplying each model with an existing C benchmark suite~\cite{bader2023tc} to reduce the risk of memorized training data reproduction.
...
\textbf{Anthropic Claude Opus 4.6} (released February 5, 2026~\cite{anthropic2026opus46}) is Anthropic's most capable model, designed for superior coding, complex reasoning, and agentic workflows with a 1M token context window. Compared to Claude Sonnet 4 used in the original paper, it achieves better results on Terminal-Bench 2.0, leading us to expect stronger implementations.
...
\begin{itemize}
  ...
  \item \textbf{Correctness:} The code must produce correct outputs on all test cases.
...
OpenAI GPT-5.3-Codex & 0.00 \\
\bottomrule
\end{tabular}
\end{table}

The efficiency rates reflect the RTU results directly. Gemini 3.1 Pro achieves the highest total efficiency rate of 2.63, contributing rates across all three algorithms. Claude Opus 4.6 contributes only through its RTU triangle counting implementation, yielding a rate of 0.15, while GPT-5.3-Codex contributes nothing. Compared to the original paper's best result of 3.11 (Claude Sonnet 4 Extended), Gemini 3.1 Pro's 2.63 is competitive, suggesting that the newer Gemini model has significantly closed the gap with Anthropic's models observed in the original study. However, the low rates of the other models reflect a key limitation of the cross-language setting, where small compilability failures, such as using accessor methods instead of direct attribute access, caused by incorrect API assumptions, disproportionately penalizes the LLMs. Furthermore, with only three algorithms evaluated, a single failure has an outsized effect on the total score. Evaluating a broader algorithm suite would likely produce more differentiated and balanced results.
...
This project evaluated three LLMs on their ability to generate efficient Java graph algorithm implementations when provided with C source files as context. In the optimization approach, each model was able to generate a correct solution, with GPT-5.3-Codex and Claude Opus 4.6 achieving approximately $91\times$ speedup over the naive baseline. In contrast, Gemini 3.1 Pro implemented an in-place algorithm that was slower than its competitors but still outperformed the baseline with a speedup of roughly $26\times$. Despite these strong results, Gemini was the only model to continue this strong trend in the algorithm synthesis with 100\% RTU implementations for Triangle Counting, Graph Diameter, and Clique Number. While the Codex model from OpenAI failed to produce RTU Java code, due to lack of compilability and timeliness, Claude Opus was able to generate a correct Triangle Counting routine that was even faster than the one from Gemini. 

The central challenge in cross-language code generation lies in the API assumptions. Unlike the original paper, where models could directly reuse field names and function signatures from the provided C source, our setting required each model to abstract away the source language and infer a plausible Java API independently. Even state-of-the-art models failed at this. Furthermore, it is difficult to assess whether models genuinely reasoned from the provided C context or simply reproduced Java graph algorithm patterns seen during training. This is an inherent limitation of any LLM evaluation without training data transparency. Furthermore, the introduced efficiency metric itself amplified small differences in this setting. With only three algorithms evaluated, a single compilability failure has an outsized effect on the total score, and the RTU definition does not distinguish between a near-miss requiring one-line fix and a fundamentally incorrect implementation. A broader algorithm suite and a partial-credit RTU metric would produce more differentiated and informative results.

Future research could expand the evaluation suite to include edge and vertex connectivity, to offer a more nuanced assessment of model performance in the cross-language setting~\cite{llmgen2025}. Furthermore, investigating the translation across more divergent languages, such as C and Rust or COBOL and Python, could reveal whether the difficulty of the API integration scales with conceptual difference between programming languages. Finally, implementing iterative feedback loops, where models see compiler diagnostics and are allowed self-correction, provides a promising direction to overcome the pitfalls of the current efficiency rate metric.
...
I hereby declare that I have made limited use of generative artificial intelligence during the preparation of this project. Specifically, Claude Sonnet 4.6 and Gemini 2.5 Flash were used to aid the translation of the C infrastructure files into Java (\texttt{Graph.java}, \texttt{BFS.java}, \texttt{Queue.java}, \texttt{Main.java}). Furthermore, they were used for guidance on overall project setup and Java compilation. All experimental results, analysis, and the written content are my own work.
